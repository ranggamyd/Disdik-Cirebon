			...
			<div class="card">
				...
				<div class="card-body">
					...
					<div class="form-group">
						...
						<input type="text" name="nama_persyaratan" placeholder="Nama Persyaratan" class="form-control" value="{{ $persyaratan->nama_persyaratan }}" required>
...
<script>
	...
	$(function(){
		...
		$form.find(`[name="id_pendidikan"]`).val(`{{ $persyaratan->id_pendidikan }}`).trigger('change')

		$form.find(`[name="tipe_input"]`).select2({
			'placeholder' : '- Pilih Tipe Input -'
		})
		$form.find(`[name="is_required"]`).val(`{{ $persyaratan->is_required }}`).trigger('change')
		...
		$form.on('submit', function(e){
			...
			ajaxSetup();
			$.ajax({
				url: `{{ route('persyaratan.update', $persyaratan->id_persyaratan) }}`,
				method: 'PUT',
				data: formData,
				dataType: 'json'
			})
		...
		$form.find(`[name="tipe_input"]`).val(`{{ $persyaratan->tipe_input }}`).trigger('change')
		setTimeout(() => {
			$form.find(`[name="opsi_multiple"]`).val(`{{ $persyaratan->opsi_multiple }}`)
		}, 500)
